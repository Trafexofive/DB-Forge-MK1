...
RESPONSE=$("$CLIENT_SCRIPT" get_root)
...
RESPONSE=$("$CLIENT_SCRIPT" spawn_db "$DB_NAME")
...
"$CLIENT_SCRIPT" spawn_db "$DB_NAME_2" > /dev/null
RESPONSE=$("$CLIENT_SCRIPT" list_dbs)
...
RESPONSE=$("$CLIENT_SCRIPT" prune_db "$DB_NAME_2")
...
TABLE_NAME="users"
"$CLIENT_SCRIPT" spawn_db "$DB_NAME_DATA" > /dev/null
...
EOF
RESPONSE=$("$CLIENT_SCRIPT" create_table "$DB_NAME_DATA" "$TABLE_NAME" "scripts/create_table_payload.json")
...
EOF
RESPONSE=$("$CLIENT_SCRIPT" insert_rows "$DB_NAME_DATA" "$TABLE_NAME" "scripts/insert_payload.json")
...
RESPONSE=$("$CLIENT_SCRIPT" get_rows "$DB_NAME_DATA" "$TABLE_NAME")
...
RESPONSE=$("$CLIENT_SCRIPT" get_rows "$DB_NAME_DATA" "$TABLE_NAME" "age=24")
...
EOF
RESPONSE=$("$CLIENT_SCRIPT" raw_query "$DB_NAME_DATA" "scripts/query_payload.json")
...
EOF
RESPONSE=$("$CLIENT_SCRIPT" raw_query "$DB_NAME_DATA" "scripts/insert_raw_payload.json")
...
EOF
RESPONSE=$("$CLIENT_SCRIPT" raw_query "$DB_NAME_DATA" "scripts/update_raw_payload.json")
...
EOF
RESPONSE=$("$CLIENT_SCRIPT" raw_query "$DB_NAME_DATA" "scripts/delete_raw_payload.json")
...
EOF
RESPONSE=$("$CLIENT_SCRIPT" raw_query "$DB_NAME_DATA" "scripts/error_query_payload.json")
...
RESPONSE=$("$CLIENT_SCRIPT" get_rows "$NON_EXISTENT_DB" "some_table")
...
"$CLIENT_SCRIPT" prune_db "$DB_NAME" > /dev/null
"$CLIENT_SCRIPT" prune_db "$DB_NAME_DATA" > /dev/null
"$CLIENT_SCRIPT" prune_db "test_db_spawn_01" > /dev/null
"$CLIENT_SCRIPT" prune_db "test_db_spawn_02" > /dev/null
"$CLIENT_SCRIPT" prune_db "test_db_data_01" > /dev/null
"$CLIENT_SCRIPT" prune_db "test_db_raw_sql" > /dev/null
